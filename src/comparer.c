...
unsigned int hash(unsigned char r, unsigned char g, unsigned char b) {

    return r + 256*g + 65280*b;
}

int read_INT(const unsigned char *buf, unsigned int *offset) {
...
unsigned short read_USHORT(const unsigned char *buf, unsigned int *offset) {

    *offset += 2;
    return buf[*offset - 1] << 8 | buf[*offset - 2];
}

unsigned int read_UINT(const unsigned char *buf, unsigned int *offset) {
...
int main(int argc, char **argv) {
    ...
    unsigned int offset_first = 0, offset_second = 0;
    ...
    if (meta_first.bits_per_pixel == 8) {
        ...
        unsigned int row_size = (meta_first.bits_per_pixel * meta_first.bitmap_width + 31) / 32 * 4;
    ...
    if (meta_first.bits_per_pixel == 8) {
        ...
        for (unsigned int i = 4; i < 1025; i += 4) {
            ...
            if (buf[i - 1] != 0) {
                ...
                fprintf(stderr, "First file has wrong color palette!");
                return -1;
            }
        }
        free(buf);
        ...
        first_bmp.pixel_array = malloc(meta_first.image_size * 4);
        for (unsigned int i = 0; i < meta_first.image_size; i++) {
        ...
        unsigned int row_size = (meta_first.bits_per_pixel * meta_first.bitmap_width + 31) / 32 * 4;
        first_bmp.pixel_array = malloc(abs(meta_first.bitmap_height) * meta_first.bitmap_width * sizeof(int));
        unsigned char *buf = malloc(meta_first.image_size);
        fread(buf, meta_first.image_size, 1, first);
        unsigned int counter = 0; unsigned int global_i = 0;
        for (unsigned j = 0; j < abs(meta_first.bitmap_height); j++) {

            for (unsigned int i = 0; i < meta_first.bitmap_width; i++) {
    ...
    if (meta_second.bits_per_pixel == 8) {
        ...
        for (unsigned int i = 4; i < 1025; i += 4) {
            ...
            if (buf[i - 1] != 0) {
                ...
                fprintf(stderr, "Second file has wrong color palette!");
                return -1;
            }
        }
        free(buf);
        buf = malloc(meta_second.image_size);
        fread(buf, meta_second.image_size, 1, first);
        second_bmp.pixel_array = malloc(meta_second.image_size);
        for (unsigned int i = 0; i < meta_second.image_size; i++) {
        ...
        unsigned int row_size = (meta_second.bits_per_pixel * meta_second.bitmap_width + 31) / 32 * 4;
        second_bmp.pixel_array = malloc(row_size * abs(meta_second.bitmap_height) * sizeof(int));
        unsigned char *buf = malloc(meta_second.image_size);
        fread(buf, meta_second.image_size, 1, second);
        unsigned int counter = 0; unsigned int global_i = 0;
        for (unsigned j = 0; j < abs(meta_second.bitmap_height); j++) {

            for (unsigned int i = 0; i < meta_second.bitmap_width; i++) {
    ...
    int all_pixels = abs(meta_first.bitmap_height) * meta_first.bitmap_width;
    for (unsigned int y = 0; y < abs(meta_first.bitmap_height); y++) {

        for (unsigned int x = 0; x < meta_first.bitmap_height; x++) {
